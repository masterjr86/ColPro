...
db = client['hh_vacancys']
django_vacancys = db.django_vacancys


db.django_vacancys.insert_many(vacancys)

def salary_search(salary):
    input_salary = int(input('Введите З/П:'))
    results = db.django_vacancys.find(
        {'salary_min': {'$gt': input_salary}}, {'link': True, 'name': True, }
    )
